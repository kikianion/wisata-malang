...
switch ($_GET[act]) {
  ...
  default:
    ...
    while ($r = mysql_fetch_array($tampil)) {
      ...
      for ($i = 0; $i < count($q); $i++) {
        ...
        $str .= "<a class='xbtn xbtn-xs xbtn-danger' style='' href=$aksi?page=penginapan_wisata&act=hapus&idinapurut=$i&id=$wisata> X </a>" . $bb['nama'] . "&nbsp;<br>";
      }
      $body .= "<tr><td>$no</td>
	  <td>$aa[nama]</td>
                <td><a href=?page=penginapan_wisata&act=tambahpenginapan&id=$wisata&nama=$aa[nama]>+ Tambah</a><br>$str</td>
		        </tr>";
      $no++;
    }
    $body .= "</table>";
  ...
  case "tambahpenginapan":
    ...
    $body .= "<h2>Tambah Penginapan Wisata</h2>
		  ...
		  <tr><td><select name='penginapan' class='form-control'>
          ";
